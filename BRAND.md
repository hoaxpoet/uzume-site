...
- **Engine footage is not chrome, and may loop.** The ban above is on chrome that moves on its own and competes with the output it exists to frame. A loop of real engine output *is* the output: it is the argument the page is making, not decoration around it, and it stays exempt even when the page crops, positions, or veils it to seat type — those are presentation decisions applied to the work, not a reclassification of it. Two conditions hold the exemption. The footage must be **real capture, published unedited, and the same scene named and playable elsewhere on the site** — so the claim is verifiable, if not in the frame itself. And any loop that starts on its own must carry a visible, labelled control that stops it (WCAG 2.2.2), which is a caused control and therefore permitted motion in its own right. A loop that fails either condition is chrome again.

  *Amended 2026-09-23, Matt's call. The first condition previously required the footage to be **labelled as engine output**, in frame. The hero credit line that satisfied it was cut as pretentious, and rather than leave a condition quietly unmet the rule now rests on the scene being identifiable on `/gallery` instead. The cost is real and is recorded rather than argued away: an unlabelled hero can be read as stock photography — a critique on this date found exactly that — and the page's “this is what the app draws” argument is weaker for it. What the condition still forbids is footage nobody can trace: a loop whose scene appears nowhere else on the site is chrome, however real its provenance.*
