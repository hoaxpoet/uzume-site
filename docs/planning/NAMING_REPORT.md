...
- **Domains (corrected 2026-08-12 at the registrar — ground truth):** `uzume.app` — **registered and parked** (the earlier "available" call rested on WHOIS/DNS signals that turn out to be unreliable for Google-registry TLDs; registrar search is the only trustworthy check for .app). `uzume.io` — **available, ~$50/yr — the canonical domain.** `uzume.com` — held since 1998 by **Uzume Taiko** (active Canadian taiko ensemble); expires 2026-08-27, likely renews. `uzume.co` — dead site (defunct task app). getuzume.com/.app available but declined — "get" prefixes are permanent brand debt.
...
- **Registration plan (revised 2026-08-12):** `uzume.io` (canonical) at ~$50/yr; watch/backorder on **both** `uzume.app` (parked — parked domains lapse constantly) and `uzume.com` (expiry 2026-08-27). Decision after uzume.app turned out parked: stay Uzume — the name won on myth and phonetics, not domains; Hotarugari (.com/.app/.io all free) remains the recorded fallback.
...
Rename touches, in rough dependency order: **register `uzume.io` first** (before any public commits reference the name; watch/backorder `uzume.app` and `uzume.com`); GitHub repo rename to `hoaxpoet/uzume` (GitHub auto-redirects old URLs); bundle ID (`com.phosphene.app` → `io.uzume.mac` — reverse-DNS of the owned domain uzume.io; avoid `com.uzume.*` and `app.uzume.*`, both third-party domains) and code-signing identity; the `com.phosphene.*` logger subsystems (→ `io.uzume.*`) and the `~/Library/Application Support/Phosphene/` hot-reload path (a migration shim or a clean break is fine pre-beta); docs sweep (README, CONTRIBUTING, docs/, CLAUDE.md, prompts); `tccutil` invocations in RUNBOOK; and the projectM/Milkdrop attribution posture carries over unchanged. None of this is hard pre-launch; all of it is annoying post-launch. The name's story is also usable product vocabulary — Omoikane as the orchestrator codename, Pythagoras as the onboarding persona, the Iwato imagery (mirror, dawn, rhythm) for the brand — worth a deliberate UX-copy pass in a later increment rather than leaving the name decorative. The full RN.1 session prompt encodes the mechanical scope.
