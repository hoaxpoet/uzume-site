...
that setup. You then log in to Spotify once, in your own browser, and the copy
you built remembers it from then on; your private and collaborative playlists
are readable that way, not only public ones.
...
scenes. [Contributing a scene](/docs/contributing/) is the walkthrough — what the
two files hold, how to get one rendering, and what the project's gates check.
