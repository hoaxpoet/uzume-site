...
Website icons use one consistent set with a 1.75–2 px stroke at 24 px. Native controls use SF Symbols and system-provided icons. The full First Opening icon is used at 64 px and above; the supplied art-directed crop is used at 16–32 px.

**The wordmark.** The outlined SVG exists because the wordmark must not be approximated in whatever face happens to be available. Where Alumni Sans is actually served — the website, which loads it in `src/styles/fonts.css` — that risk is gone: the artefact is outlined Alumni Sans SemiBold, so the same letterforms set live at 600 with `--tracking-display` are the same drawing. Set it live there. It is then selectable, it scales with the reader's text settings, it survives translation, and it can be the page's `h1` — and the website's header handoff is only possible because the hero's name and the header's are the same live glyphs sharing one timeline. Use the outlined SVG wherever the face cannot be guaranteed — email, third-party embeds, favicons, anything rendered outside a page that serves the fonts — at 96 px wide or more. A surface that sets it live owes a `font-display` and fallback-metrics check, because a failed font load turns the wordmark into a different drawing at display size.
...
**Website (Persuade).** A marketing surface may carry authored motion the app never would, and one deliberate moment is the target rather than the ceiling. Motion stays caused: it accompanies arrival, a control's own state, or a scroll position that corresponds to a real handoff. Never looping brand chrome, decorative pulsing, synchronized section entrances, or parallax — chrome that moves on its own competes with the engine footage it exists to frame. Engine footage itself is exempt and may loop: it is the thing being framed, not the frame. The exemption is conditional — the footage is real capture, labelled as engine output, and any self-starting loop carries a visible, labelled stop control (WCAG 2.2.2, which is not conditioned on `prefers-reduced-motion`). See BRAND.md for the full statement.
...
- **Don't** use vaporwave glow, generic AI shimmer, DAW skeuomorphism, stock music imagery, generic waveforms, or looping brand chrome — a looping frame of real engine output is not chrome and is exempt on the conditions above.
